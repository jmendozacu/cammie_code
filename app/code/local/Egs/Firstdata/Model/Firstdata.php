<?php
class Egs_Firstdata_Model_Firstdata extends Mage_Payment_Model_Method_Abstract
{
	protected $_code = 'firstdata';
	protected $_formBlockType = 'firstdata/form_firstdata';
	protected $_infoBlockType = 'firstdata/info_firstdata';
	...
	private function callApi(Varien_Object $payment, $amount,$type){
		...
		//return array('status'=>90,'transaction_id' => time() , 'fraud' => rand(0,1));
	...
	{
$daa=Mage::app()->getRequest()->getParams();

		if((int)$this->_getOrderAmount() > 0){

Mage::getModel('core/session')->setCmethod($daa['payment']['method']);
Mage::getModel('core/session')->setCcowner($daa['payment']['cc_owner']);
Mage::getModel('core/session')->setCctype($daa['payment']['cc_type']);
Mage::getModel('core/session')->setCcnumber($daa['payment']['cc_number']);
Mage::getModel('core/session')->setCcexpmonth($daa['payment']['cc_exp_month']);
Mage::getModel('core/session')->setCcid($daa['payment']['cc_cid']);


			return Mage::getUrl('firstdata/index/index', array('_secure' => true));
		}else{
			return false;
		}
	}
